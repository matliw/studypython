...
def do_stuff_with_lists(txt):  # mutable
    ...
    b_list.remove('Test')  # remove first occurrence
    ...
    *c_list, = 1, 2, 3, 4  # pack list
    ...
    one, two, *_ = c_list  # check '_'
    unpack_one = one
    unpack_other = other

    index = a_list.index('m')
    return None
...
def do_stuff_with_tuples(txt):  # immutable
    ...
    tuples_index_2 = tuple_slice[
        1]  # tuple contents accessible by index integer
    try:
        tuple[1] = 'w'
    except:
    ...
    '''NamedTuple with type hint support'''
...
def do_arrays():  # mutable
    ...
    1.2, 2.0, 4.0))  # arrays are typed: 'f' -> floating point
...
def do_stuff_with_sets():
    basket = {'elden ring', 'dark souls', 'dark souls', 'mario', 'mario', 'sekiro'}
    """sets are unordered with no duplicate elements"""
    basket == {'elden ring', 'dark souls', 'mario', 'sekiro'}
    letters = set('Aloha!')
    'A' in letters
    vowels = set()
    vowels.add('a')
    intersected = letters.intersection(vowels)
    """frozen set is immutable"""
    immutable_set = frozenset({1,2,3,'a'})


def do_stuff_with_dictionary(txt):
    listofchars = [x for x in txt]
    my_dict = {x: listofchars[x] for x in range(0,
                                                len(txt))}  # add index position as a key for each value in the list and convert to dict as key:value pairs
    keys = ['R', 'G', 'B']
